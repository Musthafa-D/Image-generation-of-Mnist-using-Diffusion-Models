...
class UNet_Latent(nn.Module):
    def __init__(self, initial_in_channels=3, final_out_channels=3, time_dim=256, hidden_channels=16, device="cuda"):
        ...
        self.encoder1 = Encoder_block(hidden_channels, hidden_channels)
        self.encoder2 = Encoder_block(hidden_channels, hidden_channels*2)
        self.encoder3 = Encoder_block(hidden_channels*2, hidden_channels*2)

        # bottleneck
        self.bmid1 = ConvBlock(hidden_channels*2, hidden_channels*4)
        self.bmid2 = ConvBlock(hidden_channels*4, hidden_channels*4)
        self.bmid3 = ConvBlock(hidden_channels*4, hidden_channels*2)

        # upsample
        self.decoder1 = Decoder_block(hidden_channels*4, hidden_channels)
    ...
    def noise_prediction(self, ins, t):
        ...
        x1 = self.inc(ins)
        #print(f"x1: {x1.shape}")
        x2 = self.encoder1(x1, t, 0)
        #print(f"x2: {x2.shape}")
        x3 = self.encoder2(x2, t, 0)
        #print(f"x3: {x3.shape}")
        x4 = self.encoder3(x3, t, 1)
        #print(f"x4: {x4.shape}")
        ...
        x = self.decoder1(x4, x3, t, 1)
        #print(f"x: {x.shape}")
        x = self.decoder2(x, x2, t, 0)
        #print(f"x: {x.shape}")
        x = self.decoder3(x, x1, t, 0)
        #print(f"x: {x.shape}")
        output = self.outc(x)
...
class UNet_conditional_Latent(nn.Module):
